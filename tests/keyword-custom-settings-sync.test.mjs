...
test('关键词自定义推广智能出价不会把预算类型强制改为 normal', () => {
  const block = getPruneKeywordCampaignForCustomSceneBlock();
  assert.doesNotMatch(
    block,
    /forceKeywordDailyBudget/,
    '仍存在关键词自定义推广智能出价强制每日预算变量'
  );
  assert.doesNotMatch(
    block,
    /if \(forceKeywordDailyBudget\) normalizedDmcType = 'normal';/,
    '关键词自定义推广智能出价仍会强制 normal 预算类型'
  );
  assert.match(
    block,
    /let normalizedDmcType = normalizeKeywordBudgetType\(out\.dmcType\)[\s\S]*normalizeKeywordBudgetType\(DEFAULTS\.dmcType\)[\s\S]*'day_average';/,
    '关键词预算类型缺少 day_average 默认兜底'
  );
  assert.match(
    block,
...
test('关键词自定义推广提交流程会同步计划MCB出价模型字段', () => {
  ...
  assert.match(
    ...
    'MCB 兜底字段匹配未覆盖 bid_model 这类下划线变体'
  );
  assert.match(
    source,
    /const removeKeywordMcbBidModelFields = \(campaign = \{\}\) => \{[\s\S]*KEYWORD_CUSTOM_CAMPAIGN_EXTRA_KEY_RE\.test\(key\)[\s\S]*delete campaign\[key\];/,
    '缺少统一清理 MCB/bidModel 变体字段的 helper'
  );
  assert.match(
    block,
    /'planMcbBidModel'/,
  ...
  assert.match(
    block,
    /else if \(keywordClickContract\) \{[\s\S]*removeKeywordMcbBidModelFields\(out\);/,
    '点击量目标未统一清理 mcb/bidModel 字段，可能触发手动出价降级'
  );
});
...
test('关键词自定义推广收藏加购目标成本为空不兜底，填写时必须 >=5', () => {
  ...
  assert.match(
    source,
    /const normalizeOptionalNumber = \(value\) => \{[\s\S]*?if \(typeof value === 'string' && !value\.trim\(\)\) return NaN;[\s\S]*?\};[\s\S]*?const resolveOptionalNumberSeed = \(\.\.\.values\) => \{/,
    '缺少可区分空字符串与 0 的可选数值解析 helper'
  );
  assert.match(
    block,
  ...
  assert.match(
    block,
    /const keywordMaxAmountCopyContract = isCopyCurrentPlan[\s\S]*?String\(out\.bidType \|\| input\?\.bidType \|\| ''\)\.trim\(\)\.toLowerCase\(\) === 'max_amount';/,
    '当前计划复制未优先识别源计划 max_amount 合同'
  );
  assert.match(
    block,
    /const keywordFavCartContract = !keywordMaxAmountCopyContract && keywordBidTargetCode === 'fav_cart';/,
    'max_amount 复制不应进入收藏加购控目标成本合同'
  );
  assert.match(
    block,
    /if \(keywordMaxAmountCopyContract\) \{[\s\S]*out\.bidType = 'max_amount';[\s\S]*out\.setSingleCostV2 = false;[\s\S]*delete out\.constraintType;[\s\S]*delete out\.constraintValue;[\s\S]*removeKeywordMcbBidModelFields\(out\);/,
    'max_amount 复制应关闭目标成本并统一清理 constraint 与 MCB 字段'
  );
  assert.match(
    block,
    /if \(keywordMaxAmountCopyContract\) \{[\s\S]*const nativeMaxAmountTarget = keywordBidTargetCode === 'fav_cart' \? 'coll_cart' : keywordBidTargetCode;[\s\S]*out\.bidTypeV2 = out\.bidTypeV2 \|\| 'smart_bid';[\s\S]*out\.bidTargetV2 = nativeMaxAmountTarget;[\s\S]*out\.optimizeTarget = nativeMaxAmountTarget;/,
    'max_amount 复制未保留 smart_bid 或未把 fav_cart 原生映射为 coll_cart'
  );
  assert.match(
    block,
    /else if \(!keywordMaxAmountCopyContract && resolvedMcbBidModel\) \{/,
    'max_amount 复制清理 mcb 后不应被默认 MCB 逻辑回填'
  );
  assert.match(
    block,
    /const favCartConstraintSeed = resolveOptionalNumberSeed\([\s\S]*?out\.singleCostV2,[\s\S]*?out\.constraintValue[\s\S]*?\);[\s\S]*?const hasFavCartConstraintValue = Number\.isFinite\(favCartConstraintSeed\);/,
...
test('关键词自定义推广获取成交量会对齐原生 dir_conv 合同并清理冲突字段', () => {
  ...
  assert.match(
    block,
    /if \(keywordConvContract\) \{[\s\S]*removeKeywordMcbBidModelFields\(out\);/,
    'conv 合同未统一清理 mcb/bidModel 字段'
  );
});

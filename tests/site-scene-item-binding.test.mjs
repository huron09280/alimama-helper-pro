...
function extractSceneSettingMappingBlock() {
  ...
  assert.ok(start > -1 && end > start, '无法定位场景设置映射分支');
  return source.slice(start, end);
}
...
test('货品全站推广关闭一键起量与无归属计划组按原生结构提交', () => {
  const siteBuildBlock = extractSiteSceneBuildBlock();
  assert.match(
    siteBuildBlock,
    /merged\.campaign\.quickLiftBudgetCommand = \{ quickLiftSwitch: 'false' \};/,
    '货品全站推广关闭一键起量时应保留 quickLiftBudgetCommand.quickLiftSwitch=false'
  );

  const mappingBlock = extractSceneSettingMappingBlock();
  assert.match(
    mappingBlock,
    /不归属任何计划组[\s\S]*?applyCampaignExact\('campaignGroupId', null, campaignGroupEntry\.key, campaignGroupEntry\.value\);/,
    '计划组“不归属任何计划组”未映射 campaignGroupId=null'
  );
  assert.match(
    mappingBlock,
    /applyCampaignExact\('campaignGroupName', '', campaignGroupEntry\.key, campaignGroupEntry\.value\);/,
    '计划组“不归属任何计划组”未清空 campaignGroupName'
  );
});

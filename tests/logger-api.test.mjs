...
test('主助手 DOM 观察覆盖属性和文本变化，兼容 SPA 表格复用', () => {
  ...
  assert.match(entrypointBlock, /observer\.observe\(document\.body,\s*\{[\s\S]*childList:\s*true,[\s\S]*subtree:\s*true,[\s\S]*attributes:\s*true,[\s\S]*attributeFilter:\s*\['class',\s*'style',\s*'aria-hidden'\],[\s\S]*characterData:\s*true[\s\S]*\}\);/, 'MutationObserver 未覆盖属性/文本变化');
});

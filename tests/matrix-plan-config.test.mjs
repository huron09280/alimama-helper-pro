...
test('商品池变化会回填已有 material_id 维度，避免矩阵商品维度空值', () => {
  ...
  assert.match(
    source,
    /const collectDefaultSelectedMatrixMaterialDimensionValues = \(itemList = \[\]\) => \(\s*collectMatrixMaterialDimensionValues\(itemList\)\.slice\(0,\s*1\)\s*\);/,
    '矩阵商品维度缺少默认只选首个商品的 helper'
  );
  assert.match(
    source,
    /const defaultMaterialValues = preset\.key === 'material_id'[\s\S]*?collectDefaultSelectedMatrixMaterialDimensionValues\(itemList\)/,
    '新增商品维度草稿时未默认选中首个商品'
  );
  assert.match(
    source,
    /const syncMatrixMaterialDimensionValues = \(matrixConfig = \{\}, itemList = \[\], sceneName = ''\) => \{[\s\S]*?const defaultSelectedValues = collectDefaultSelectedMatrixMaterialDimensionValues\(itemList\);[\s\S]*?dimension\.key !== 'material_id'[\s\S]*?const validValues = currentValues\.filter\(value => materialValueSet\.has\(value\)\);[\s\S]*?const nextValues = validValues\.length \? validValues : defaultSelectedValues;/,
...
test('矩阵页支持一键补齐推荐 5 维并自动聚焦新增卡片', () => {
  ...
  assert.match(source, /id="am-wxt-matrix-generate-plans"/, '矩阵页缺少生成计划按钮');
  ...
  assert.match(
    source,
    /matrixApplyRecommendedBtn: overlay\.querySelector\('#am-wxt-matrix-apply-recommended'\),[\s\S]*?matrixGenerateBtn: overlay\.querySelector\('#am-wxt-matrix-generate-plans'\),[\s\S]*?matrixClearBtn: overlay\.querySelector\('#am-wxt-matrix-clear-dimensions'\),[\s\S]*?matrixActionNote: overlay\.querySelector\('#am-wxt-matrix-action-note'\)/,
...
test('矩阵物化后的计划会绑定回具体商品，避免首页再次按商品池展开', () => {
  assert.match(source, /const resolveStrategyBoundMaterialId = \(strategy = \{\}\) => String\(toIdValue\(/, '缺少计划级商品绑定 helper');
  assert.match(
    source,
    /const resolveStrategyBoundItemList = \(strategy = \{\}, itemList = \[\]\) => \{[\s\S]*?if \(!materialId\) return normalizedItems;[\s\S]*?const matchedItem = resolveMatrixBoundItem\(materialId,\s*normalizedItems\);[\s\S]*?throw new Error\(`计划绑定商品已失效：\$\{materialId\}`\);/,
    '计划级商品绑定缺少商品池解析与失效校验'
  );
  assert.match(
    source,
    /const materializeStrategyListFromPlans = \(plans = \[\]\) => normalizeStrategyList\([\s\S]*?materialId,[\s\S]*?materialName,[\s\S]*?item:\s*rawItem\?\.materialId \? rawItem : null,[\s\S]*?matrixMaterialized:\s*true/,
    '矩阵物化回首页时未保留商品绑定快照'
  );
  assert.match(
    source,
    /const strategyBoundMaterialId = resolveStrategyBoundMaterialId\(strategy\);[\s\S]*?const strategyItemList = resolveStrategyBoundItemList\(strategy,\s*wizardState\.addedItems\);[\s\S]*?const shouldAppendItemIndex = strategyItemList\.length > 1 \|\| \(!strategyBoundMaterialId && wizardState\.addedItems\.length > 1\);/,
    'buildRequestFromWizard 仍未尊重计划级商品绑定'
  );
});

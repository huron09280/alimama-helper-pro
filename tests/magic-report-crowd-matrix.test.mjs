...
test('人群看板 7 天首屏先返回基础结果，省份/城市后台补齐不阻塞首渲染', () => {
  ...
  assert.match(methodBlock, /if \(days === 7\) \{[\s\S]*this\.ensureCrowdInsightExtraScopeResults\(baseResult,\s*\{\s*campaignId:\s*id,\s*metricType:\s*metric\s*\}\)[\s\S]*\.catch\(\(err\) => \{[\s\S]*省份\/城市维度后台补齐失败[\s\S]*\}\);[\s\S]*return \{[\s\S]*periodDays:\s*days,[\s\S]*groupMap:\s*baseResult\.groupMap,/, '7 天首屏未先返回基础结果或仍阻塞等待省份/城市补齐');
  assert.match(methodBlock, /if \(days === 7\)[\s\S]*return \{[\s\S]*\};[\s\S]*await this\.ensureCrowdInsightExtraScopeResults\(baseResult,\s*\{\s*campaignId:\s*id,\s*metricType:\s*metric\s*\}\);/, '非 7 天周期仍应等待省份/城市补齐以保持完整 8 维度');
});

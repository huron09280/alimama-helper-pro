...
function getMainAssistantBootstrapBlock() {
  const start = source.indexOf('const installAssistDisplayDiagnostics = () => {');
  const end = source.indexOf('// ==========================================\n// 7. 算法护航模块', start);
  assert.ok(start > -1 && end > start, '无法定位主助手启动代码块');
  return source.slice(start, end);
}
...
test('MagicReport 包含人群看板核心方法与辅助方法', () => {
  ...
  for (const method of [
    ...
    'normalizeDmpRouteToken',
    'isDmpCrowdInsightRouteToken',
...
test('DMP 人群看板复用现有矩阵框架并以自定义画像标签作为行维度', () => {
  const block = getMagicReportBlock();
  assert.match(block, /normalizeDmpRouteToken\(value = ''\)[\s\S]*replace\(\/\[_\\s\]\+\/g,\s*'-'\);/, 'DMP 路由 token 未兼容大小写、下划线或空白分隔');
  assert.match(block, /isDmpCrowdInsightRouteToken\(value = ''\)[\s\S]*normalized === 'crowd-insight' \|\| normalized\.replace\(\/-\/g,\s*''\) === 'crowdinsight';/, 'DMP 路由未兼容 crowd-insight 常见写法');
  assert.match(block, /isDmpItemInsightCrowdPage\(\)\s*\{[\s\S]*hostname[\s\S]*dmp\.taobao\.com[\s\S]*const tabParamNames = \['analysisTab', 'tab', 'activeTab', 'currentTab', 'selectedTab'\];[\s\S]*return true;/, 'DMP 页面守卫缺少宽容 tab 参数判断或缺省 tab 兜底');
  assert.match(block, /if \(text === '切换分析单品'\) return 0;/, 'DMP 入口未优先定位“切换分析单品”按钮');
  assert.match(block, /if \(text === '标杆商品池管理'\) return 2;/, 'DMP 入口缺少头部动作区 fallback 锚点');
  assert.match(block, /\.sort\(\(a, b\) => a\.priority - b\.priority\);/, 'DMP 入口锚点未按优先级选择');
...
test('DMP 入口在 SPA 路由后进入人群洞察页时会补挂按钮', () => {
  const block = getMainAssistantBootstrapBlock();
  assert.match(block, /const isDmpHostPage = \(\) => \{[\s\S]*dmp\.taobao\.com/, '主助手缺少 DMP host 分支');
  assert.match(block, /normalizeDmpRouteToken[\s\S]*replace\(\/\[_\\s\]\+\/g,\s*'-'\);/, 'DMP 路由监听缺少 tab token 兼容');
  assert.match(block, /const tabParamNames = \['analysisTab', 'tab', 'activeTab', 'currentTab', 'selectedTab'\];[\s\S]*if \(tabValues\.length\) \{[\s\S]*return tabValues\.some\(isDmpCrowdInsightRouteToken\);[\s\S]*\}\s*return true;/, 'DMP 路由监听未兼容 tab 参数变体或缺省 tab');
  assert.match(block, /const installDmpCrowdMatrixRouteWatcher = \(\) => \{/, '缺少 DMP 路由监听安装函数');
  assert.match(block, /ensureDmpEntryForCurrentRoute[\s\S]*if \(isDmpItemInsightCrowdPage\(\)\) \{[\s\S]*MagicReport\.initDmpCrowdMatrixEntry\(\);[\s\S]*return true;/, 'DMP 路由进入人群洞察页时未补执行入口初始化');
  assert.match(block, /document\.getElementById\('am-dmp-crowd-matrix-entry'\)\?\.remove\(\);[\s\S]*MagicReport\.disconnectDmpCrowdMatrixButtonObserver\?\.\(\);/, '离开 DMP 人群洞察页时未清理入口与 observer');
  assert.match(block, /hookHistoryMethod\('pushState'\);[\s\S]*hookHistoryMethod\('replaceState'\);[\s\S]*window\.addEventListener\('hashchange', checkDmpUrlChange\);[\s\S]*window\.addEventListener\('popstate', checkDmpUrlChange\);/, 'DMP 路由监听未覆盖 history/hash/popstate');
  assert.match(block, /if \(isDmpHostPage\(\)\) \{[\s\S]*installDmpCrowdMatrixRouteWatcher\(\);[\s\S]*return;[\s\S]*\}[\s\S]*UI\.init\(\);/, 'DMP host 不应落入普通 one.alimama 主助手初始化');
});

...
test('人群看板会按当前勾选人群优先排队请求，避免首屏渲染空柱', () => {
  const block = getMagicReportBlock();
  assert.match(block, /getCrowdMetricsByVisibilityPriority\(\)\s*\{/, '缺少按勾选人群排序的方法');
  assert.match(block, /return visibleMetrics\.length[\s\S]*\?\s*\[\.\.\.visibleMetrics,\s*\.\.\.hiddenMetrics\][\s\S]*:\s*this\.CROWD_METRICS\.slice\(\);/, '人群优先排序逻辑不完整');
  assert.match(block, /const prioritizedMetrics = this\.getCrowdMetricsByVisibilityPriority\(\);/, '看板加载未读取按勾选人群的优先级');
  assert.match(block, /prioritizedMetrics\.forEach\(\(metricType\) => \{[\s\S]*this\.CROWD_PERIODS\.forEach\(\(periodDays\) => \{/, '看板任务队列未按优先指标生成');
});

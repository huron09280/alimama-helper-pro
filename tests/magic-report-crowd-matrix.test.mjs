...
test('人群看板首屏不被隐藏商品识别阻塞，且省市基础查数共享 promise', () => {
  ...
  assert.match(block, /if \(initialMetrics\.includes\('itemdeal'\)\) \{[\s\S]*await this\.refreshCrowdCampaignItemOptions\(id,\s*\{\s*forceRefresh:\s*forceRefreshItems\s*\}\);[\s\S]*\} else \{[\s\S]*this\.refreshCrowdCampaignItemOptionsInBackground\(id,\s*\{\s*forceRefresh:\s*forceRefreshItems\s*\}\);/, '初次加载未避免非商品成交场景等待商品列表识别');

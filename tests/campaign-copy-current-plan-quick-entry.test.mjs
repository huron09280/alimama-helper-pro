...
const extractObjectMethod = (source, methodName) => {
    const marker = new RegExp(`(^|\\n)\\s*${methodName}\\s*\\(`);
    const match = marker.exec(source);
    const start = match ? match.index + match[1].length : -1;
    assert.notEqual(start, -1, `缺少对象方法：${methodName}`);
    const signatureStart = source.indexOf('(', start);
    assert.notEqual(signatureStart, -1, `对象方法缺少参数列表：${methodName}`);
    let parenDepth = 0;
    let signatureEnd = -1;
    ...
    assert.notEqual(signatureEnd, -1, `对象方法参数列表未闭合：${methodName}`);
    const bodyStart = source.indexOf('{', signatureEnd);
    assert.notEqual(bodyStart, -1, `对象方法缺少方法体：${methodName}`);
    let depth = 0;
    ...
    assert.fail(`对象方法未闭合：${methodName}`);
};

const buildQuickEntryMethodHost = (methodName) => {
    const methodSource = extractObjectMethod(quickEntry, methodName);
...
test('关键词人群解析兼容绑定行和直接 crowd 行并过滤源单元', () => {
    const host = buildQuickEntryMethodHost('extractCopyCrowdListFromPayload');
    const payload = {
        data: {
            list: [
                {
    ...
    const out = host.extractCopyCrowdListFromPayload(payload, '69514602419', '69510831221');
    assert.deepEqual(
...
test('复制入口读取当前计划详情并调用受控 API', () => {
    ...
    assert.match(quickEntry, /queryAdgroupDetail\(id,\s*resolvedAdgroupId,\s*targetBizCode,\s*authContext\)/, '复制前应按解析出的单元 ID 读取单元详情');
    assert.match(quickEntry, /queryBidwordList\(id,\s*currentAdgroupId,\s*targetBizCode,\s*authContext\)/, '关键词复制前应读取原生 bidword/findList 关键词明细');
    assert.match(quickEntry, /\/bidword\/findList\.json\?\$\{query\.toString\(\)\}/, '源关键词应来自原生 bidword/findList 接口');
    assert.match(quickEntry, /campaignIdList:\s*\[normalizedCampaignId\][\s\S]*?adgroupIdList:\s*\[normalizedAdgroupId\]/, '关键词明细查询必须同时绑定源 campaignId 和 adgroupId');
    assert.match(quickEntry, /adgroup\.wordList = wordList;/, '源关键词明细应写回 adgroup.wordList 参与复制');
    assert.match(quickEntry, /查询关键词明细失败，已取消复制/, '源关键词读取失败时应 fail-fast，避免创建无关键词半成品');
    assert.match(quickEntry, /queryWordPackageList\(id,\s*currentAdgroupId,\s*targetBizCode,\s*authContext\)/, '关键词复制前应读取原生词包明细');
    assert.match(quickEntry, /\/wordpackage\/findList\.json\?\$\{query\.toString\(\)\}/, '源词包应来自原生 wordpackage/findList 接口');
    assert.match(quickEntry, /adgroup\.wordPackageList = wordPackageList;/, '源词包明细应写回 adgroup.wordPackageList 参与复制');
    assert.match(quickEntry, /查询关键词词包失败，已取消复制/, '源词包读取失败时应 fail-fast，避免创建字段缺失半成品');
    assert.match(quickEntry, /queryCampaignCrowdList\([\s\S]*?aiMaxEnabled \? '' : currentAdgroupId[\s\S]*?\)/, '非 AI点睛关键词复制应按 adgroupId 读取关键词旁边的人群');
    assert.match(quickEntry, /\/crowd\/findList\.json\?\$\{query\.toString\(\)\}/, '需求人群应来自原生 crowd/findList 接口');
    assert.match(quickEntry, /\.\.\.\(normalizedAdgroupId \? \{ adgroupId: Number\(normalizedAdgroupId\) \} : \{\}\)/, '人群查询 payload 应支持绑定源 adgroupId');
    assert.match(quickEntry, /campaign\.crowdList = campaignCrowdList;/, 'AI点睛源计划需求人群应写回 campaign.crowdList 参与复制');
    assert.match(quickEntry, /adgroup\.crowdList = campaignCrowdList;/, '自定义关键词源单元人群应写回 adgroup.crowdList 参与复制');
    assert.match(quickEntry, /查询AI点睛需求人群失败，已取消复制/, 'AI点睛源计划读不到需求人群时应 fail-fast，避免半成品复制');
    assert.match(quickEntry, /查询关键词人群失败，已取消复制/, '自定义关键词源计划人群读取失败也应 fail-fast，避免模板 rightList 被当成源人群提交');
...
test('关键词 AI点睛当前计划复制在源词包为空时补齐流量智选词包', () => {
    assert.match(
        wizardApi,
        /const COPY_KEYWORD_TRAFFIC_SMART_WORD_PACKAGE_LIST = \[[\s\S]*?wordPackageId:\s*0,[\s\S]*?wordPackageName:\s*'流量智选'[\s\S]*?strategyId:\s*1,\s*strategyName:\s*'好词优选',\s*onlineStatus:\s*1,\s*bidPrice:\s*1[\s\S]*?strategyId:\s*2,\s*strategyName:\s*'捡漏',\s*onlineStatus:\s*1,\s*bidPrice:\s*1[\s\S]*?strategyId:\s*3,\s*strategyName:\s*'类目优选',\s*onlineStatus:\s*1,\s*bidPrice:\s*1/,
...
test('关键词当前计划复制保留隐藏配置、创意和原生智能出价目标合同', () => {
    ...
    for (const field of ['wordList', 'wordPackageList', 'adzoneList', 'crowdList', 'smartCreative', 'creativeSetMode', 'openAutoCreative', 'openStaticCreative', 'creativeList', 'creativeInfo', 'materialList', 'adgroupOcpc', 'creativeTemplateType', 'creativeTemplateInfos', 'clickUrlCreativeTemplateInfos', 'blackCreativeStatus']) {
    ...
    assert.match(
        wizardApi,
        /const commonSourceCrowdList = Array\.isArray\(adgroup\.crowdList\) && adgroup\.crowdList\.length[\s\S]*?commonAdgroup\.crowdList = deepClone\(commonSourceCrowdList\);[\s\S]*?commonAdgroup\.rightList = deepClone\(commonSourceCrowdList\);/,
        '关键词复制 common rawOverrides 应把源单元 crowdList 同步到 rightList，避免模板推荐人群覆盖源人群'
    );
    assert.match(
        wizardApi,
        /const sourceCrowdList = Array\.isArray\(planAdgroup\.crowdList\) && planAdgroup\.crowdList\.length[\s\S]*?: deepClone\(commonSourceCrowdList\);[\s\S]*?planAdgroup\.crowdList = deepClone\(sourceCrowdList\);[\s\S]*?planAdgroup\.rightList = deepClone\(sourceCrowdList\);/,
        '每条复制计划应把源单元 crowdList 同步到 rightList'
    );
    assert.match(
        draftBuilder,
        /const resolveCopySourceAdgroupListField = \(field = ''\) => \{[\s\S]*?plan\?\.rawOverrides\?\.adgroup,[\s\S]*?request\?\.common\?\.rawOverrides\?\.adgroup,[\s\S]*?return \{[\s\S]*?explicit:\s*true,[\s\S]*?list:\s*Array\.isArray\(candidate\[field\]\) \? deepClone\(candidate\[field\]\) : \[][\s\S]*?\};[\s\S]*?const copySourceCrowdListState = resolveCopySourceAdgroupListField\('crowdList'\);[\s\S]*?out\.crowdList = copySourceCrowdList\.length \? deepClone\(copySourceCrowdList\) : \[];[\s\S]*?out\.rightList = copySourceCrowdList\.length[\s\S]*?\? deepClone\(copySourceCrowdList\)[\s\S]*?: \([\s\S]*?copySourceRightListState\.explicit[\s\S]*?\);/,
        '最终关键词单元裁剪应只从 rawOverrides 源字段取人群，并用源 crowdList 覆盖 rightList'
    );
    assert.match(
        draftBuilder,
        /merged\.adgroup = pruneKeywordAdgroupForCustomScene\(merged\.adgroup,\s*hasItem \? item : null,\s*\{[\s\S]*?request,[\s\S]*?plan[\s\S]*?\}\);/,
        '最终关键词单元裁剪调用必须传入 request/plan，才能识别当前计划复制'
    );
    ...
    assert.match(draftBuilder, /const original = purgeCreateTransientFields\(input\);[\s\S]*?\.\.\.original,[\s\S]*?bidPrice: toNumber\(input\.bidPrice, fallbackBid\)/, 'parseKeywords 应保留 bidStrategyInfo 等源关键词策略字段，并清理旧计划 ID');
    assert.match(draftBuilder, /const original = isPlainObject\(word\) \? purgeCreateTransientFields\(word\) : \{\};[\s\S]*?\.\.\.original,[\s\S]*?matchScope: parseMatchScope\(word\.matchScope, fallbackMatch\)/, 'appendKeywords 兜底补词应保留源关键词策略字段');
    assert.doesNotMatch(
        draftBuilder,
    ...
    assert.match(
        ...
        '关键词复制高级设置应收敛到最终提交可复用 helper'
    );
    assert.match(
        createAndSuggest,
        /const buildCreatePayload = \(entries = \[]\) => \{[\s\S]*?sceneCapabilities\.sceneName === '关键词推广'[\s\S]*?mergedRequest\?\.__copyCurrentPlan === true[\s\S]*?applyCopyKeywordAdvancedSettingsForSubmit\(solution\.campaign,[\s\S]*?request:\s*mergedRequest[\s\S]*?\);[\s\S]*?return \{[\s\S]*?bizCode:\s*runtime\.bizCode,[\s\S]*?solutionList[\s\S]*?\};[\s\S]*?\};/,
        '最终 addList 请求体应在发出前补齐关键词复制高级设置'
    );
    assert.match(
        createAndSuggest,
        /const requestOptions = resolveParallelCreateRequestOptions\(\);[\s\S]*?requestOne\(singleEndpoint,\s*runtime\.bizCode,\s*buildCreatePayload\(\[entry\]\),\s*requestOptions\)/,
...
test('关键词当前计划复制创建后必须按新单元补写源关键词', () => {
    ...
    assert.match(
        wizardApi,
        /const resolveCreatedAdgroupRefsFromCopyResult = \(result = \{\}\) => \{[\s\S]*?entry\?\.adgroupId[\s\S]*?entry\?\.adgroupIdList[\s\S]*?extractCreatedAdgroupIdsFromCreateResult\(result\)[\s\S]*?return normalizeCreatedAdgroupRefs\(refs\);[\s\S]*?const resolveCreatedAdgroupIdsFromCopyResult = \(result = \{\}\) => \{[\s\S]*?return resolveCreatedAdgroupRefsFromCopyResult\(result\)\.map\(ref => ref\.adgroupId\);/,
        '复制后应能从创建结果解析新 adgroupId，并保留兼容的 ID 列表 helper'
    );
    assert.match(
        wizardApi,
        /const buildCreatedCampaignLookupRows = \(campaignIds = \[],\s*meta = \{\},\s*result = \{\}\) => \{[\s\S]*?planName:\s*normalizeCopyLookupPlanName\(entry\?\.planName \|\| names\[index\] \|\| ''\)[\s\S]*?campaignId:\s*toPositiveIdText\(id\),[\s\S]*?planName:\s*normalizeCopyLookupPlanName\(names\[index\] \|\| ''\)/,
        '创建响应缺少 adgroupId 时应建立 campaignId + 新计划名的回查键'
    );
    assert.match(
        wizardApi,
        /const isCreatedCampaignLookupMatch = \(item = \{\},\s*lookup = \{\}\) => \{[\s\S]*?if \(lookup\.campaignId && rowCampaignId !== lookup\.campaignId\) return false;[\s\S]*?if \(lookup\.planName && rowName !== lookup\.planName\) return false;[\s\S]*?return !!rowCampaignId && \(!lookup\.planName \|\| !!rowName\);/,
        '回查新单元必须同时精确匹配 campaignId 和 planName，不能只取相似计划'
    );
    assert.match(
        wizardApi,
        /const queryCreatedAdgroupRefsByCampaignIds = async[\s\S]*?requestOne\('\/campaign\/horizontal\/findPage\.json',\s*bizCode,\s*\{[\s\S]*?adgroupRequired:\s*true,[\s\S]*?searchKey:\s*'campaignNameLike',[\s\S]*?searchValue:\s*lookup\.planName \|\| lookup\.campaignId[\s\S]*?\.filter\(item => isCreatedCampaignLookupMatch\(item,\s*lookup\)\)/,
        '回查新单元应使用带 adgroupRequired 的计划列表接口，并按新计划名筛选'
    );
    assert.match(
        wizardApi,
        /collectCreatedAdgroupRefsFromNode[\s\S]*?collectCreateAdgroupIdsFromNode\(node,\s*\[]\)[\s\S]*?collectCreateAdgroupIdsFromNode\(node\?\.lastAdgroup \|\| null,\s*\[]\)/,
        '新 adgroupId 回查应覆盖 adgroupList、lastAdgroup、adgroupIdList/adgroupIds 等返回形态'
    );
    assert.match(
        wizardApi,
        /const resolveCreatedAdgroupRefsForCopyResult = async[\s\S]*?const directRefs = resolveCreatedAdgroupRefsFromCopyResult\(result\);[\s\S]*?return queryCreatedAdgroupRefsByCampaignIds\(sceneName,\s*campaignIds,\s*meta,\s*result,\s*options\);/,
        '创建响应没有 adgroupId 时应按 campaignId/planName 回查新单元'
    );
    assert.match(
        wizardApi,
        /const appendCopiedKeywordsAfterCreate = async \(sceneName = '', result = \{\}, meta = \{\}, options = \{\}\) => \{[\s\S]*?bizCode !== 'onebpSearch'[\s\S]*?const sourceWordList = normalizeCopyKeywordWordList\(meta\?\.sourceWordList \|\| \[]\);[\s\S]*?const adgroupRefs = await resolveCreatedAdgroupRefsForCopyResult\(sceneName,\s*result,\s*meta,\s*options\);/,
        '关键词复制应在创建后准备源关键词并支持回查新单元 ID'
    );
    assert.match(
        wizardApi,
        /const presence = await queryExistingKeywordKeysForAdgroup\(ref,\s*bizCode \|\| DEFAULTS\.bizCode,\s*options\);[\s\S]*?const missingKeywords = presence\.ok[\s\S]*?getMissingCopiedKeywords\(sourceWordList,\s*presence\.existingKeys\)[\s\S]*?: sourceWordList;[\s\S]*?if \(!entries\.length\) \{[\s\S]*?reason:\s*'keywords_already_created_by_add_list'/,
        '补写关键词前应先查新单元已有关键词，addList 已落库时跳过重复 bidword/add'
    );
    assert.match(
        wizardApi,
        /failedRows\.length[\s\S]*?queryExistingKeywordKeysForAdgroup\(ref,\s*bizCode \|\| DEFAULTS\.bizCode,\s*options\)[\s\S]*?confirmRows\.every\(row => row\.ok && row\.missingWordCount === 0\)[\s\S]*?reason:\s*'keywords_confirmed_after_append_failure'/,
        'bidword/add 失败后应二次只读确认关键词是否已落库，避免误报'
    );
    assert.match(
        wizardApi,

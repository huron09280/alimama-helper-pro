...
test('关键词建计划时向单元裁剪传入当前出价模式', () => {
  assert.match(
    source,
    /pruneKeywordAdgroupForCustomScene\(merged\.adgroup,\s*hasItem \? item : null,\s*\{[\s\S]*?bidMode:\s*planBidMode,[\s\S]*?request,[\s\S]*?plan[\s\S]*?\}\)/,
    '关键词建计划未将当前出价模式和复制上下文传给单元裁剪逻辑'
  );
});
...
test('关键词复制词包提交前会清理源详情旧 ID 并保留流量智选状态', () => {
  ...
  assert.deepEqual(output, [
    {
      ...
      strategyList: [
        ...
        { strategyId: 2, strategyName: '捡漏', onlineStatus: 0, bidPrice: 1 }
      ]
    }
  ]);
  ...
  assert.deepEqual(output, [
    {
      ...
      bidPrice: 1,
      strategyList: [
        { strategyId: 1, strategyName: '好词优选', onlineStatus: 0, bidPrice: 1 },
        { strategyId: 2, strategyName: '捡漏', onlineStatus: 0, bidPrice: 1 },
        { strategyId: 3, strategyName: '类目优选', onlineStatus: 0, bidPrice: 1 }
      ]
    }
  ]);
});

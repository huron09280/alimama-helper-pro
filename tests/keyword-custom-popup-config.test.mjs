...
test('手动出价人群弹窗采用原生双栏选择器结构并保留 JSON 映射', () => {
  ...
  assert.match(
    block,
    /class="am-wxt-remove-icon-btn am-wxt-scene-crowd-remove-btn"[\s\S]*?data-scene-popup-crowd-remove="\$\{Utils\.escapeHtml\(key\)\}"[\s\S]*?aria-label="移除候选人群：\$\{name\}"[\s\S]*?title="移除人群"[\s\S]*?renderAmIcon\('close', \{ size: 12, strokeWidth: 2\.2 \}\)/,
    '人群弹窗候选移除操作未使用共享 close 图标按钮'
  );
  assert.match(
    block,
    /class="am-wxt-remove-icon-btn am-wxt-scene-crowd-remove-btn"[\s\S]*?data-scene-popup-crowd-remove="\$\{Utils\.escapeHtml\(key\)\}"[\s\S]*?aria-label="移除已选人群：\$\{Utils\.escapeHtml\(rawName\)\}"[\s\S]*?title="移除人群"[\s\S]*?renderAmIcon\('close', \{ size: 12, strokeWidth: 2\.2 \}\)/,
    '人群弹窗已选移除操作未使用共享 close 图标按钮'
  );
  assert.doesNotMatch(
    block,
    /data-scene-popup-crowd-remove="\$\{Utils\.escapeHtml\(key\)\}"[\s\S]*?>移除<\/button>/,
    '人群弹窗移除操作不应回退为文字按钮'
  );
  assert.match(
    block,

...
test('组建计划主向导内部整块灰底按小万护航方式移除', () => {
  ...
  assert.match(finalTabsBlock, /background:\s*transparent;/, '组建计划顶部页签仍在绘制整块灰白底');
  ...
  assert.match(
    source,
    /#am-wxt-keyword-modal \.am-wxt-body \{[\s\S]*?padding:\s*12px;[\s\S]*?background:\s*transparent;[\s\S]*?#am-wxt-keyword-modal \.am-wxt-workbench-tabs \{[\s\S]*?background:\s*transparent;/,
  ...
  assert.doesNotMatch(finalTabsBlock, /background:\s*(?:rgba\(255,\s*255,\s*255,\s*0\.28\)|#fff|#f8fafc);/, '组建计划顶部页签不得回退到整块灰白背景');
...
test('组建计划首页摘要卡片收敛到统一 token', () => {
  ...
  assert.match(finalHomeStatBlock, /background:\s*var\(--am26-surface-strong,/, '首页摘要卡片背景未使用 --am26-surface-strong');
  assert.match(finalHomeStatBlock, /box-shadow:\s*inset 0 1px 0 rgba\(255,\s*255,\s*255,\s*0\.32\);/, '首页摘要卡片缺少轻量内高光');
...
test('首页工具条、搜索框和计划行文字控件收敛到统一 token', () => {
  ...
  assert.match(finalSectionTitleBlock, /color:\s*var\(--am26-text,/, '计划配置标题未使用 --am26-text');
...
test('日志页预览与执行日志容器收敛到统一 token', () => {
  ...
  assert.match(finalPreviewLogCrowdBoxBlock, /border:\s*1px solid var\(--am26-border,/, '日志页摘要盒边框未使用 --am26-border');
  assert.match(finalPreviewLogCrowdBoxBlock, /background:\s*var\(--am26-surface,/, '日志页摘要盒背景未使用 --am26-surface');
  assert.match(finalPreviewLogCrowdTitleBlock, /color:\s*var\(--am26-text-soft,/, '日志页摘要标题未使用 --am26-text-soft');
  assert.match(finalPreviewLogCrowdTitleBlock, /border-bottom:\s*1px dashed rgba\(255,\s*255,\s*255,\s*0\.32\);/, '日志页摘要标题分隔线未使用浅玻璃弱分隔');

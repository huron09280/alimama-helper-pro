...
test('buildSolutionFromPlan: AI点睛当前计划复制保留高级设置资源位和屏蔽词', () => {
    assert.match(
        source,
    ...
    assert.match(
        source,
        /const ensureCopyKeywordAiMaxDefaultAdzoneList = \(campaign = \{\}, options = \{\}\) => \{[\s\S]*?isCopyCurrentPlan[\s\S]*?isCopyKeywordAiMaxEnabled\(campaign\)[\s\S]*?campaign\.adzoneList = buildDefaultKeywordSearchAdzoneList\(\);[\s\S]*?\};/,
        'AI点睛当前计划复制缺少资源位为空时的默认淘宝搜索补齐'
    );
    assert.match(
        source,
    ...
    assert.match(
        ...
        'AI点睛高级设置未收敛到资源位和屏蔽词统一提交 helper'
    );
    assert.match(
        source,
        /applyCopyKeywordAiMaxAdvancedSettingsForSubmit\(out, \{ request, plan \}\);[\s\S]*?return out;/,
        '最终关键词提交前未应用 AI点睛高级屏蔽词映射和资源位出口补齐'
    );
    assert.match(
        source,
        /const buildCreatePayload = \(entries = \[]\) => \{[\s\S]*?applyCopyKeywordAiMaxAdvancedSettingsForSubmit\(solution\.campaign,[\s\S]*?request:\s*mergedRequest[\s\S]*?\);[\s\S]*?solutionList[\s\S]*?\};/,
        '最终 addList 请求体发出前未二次校正 AI点睛高级设置'
    );
});

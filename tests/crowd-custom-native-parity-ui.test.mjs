...
test('人群推广自定义推广弹窗触发器路由到独立资源位与地域时间弹窗', () => {
  ...
  assert.match(
    renderBlock,
    /class="am-wxt-remove-icon-btn am-wxt-scene-item-remove-btn"[\s\S]*?data-scene-popup-item-remove="\$\{Utils\.escapeHtml\(itemId\)\}"[\s\S]*?aria-label="移除商品：\$\{Utils\.escapeHtml\(itemName\)\}"[\s\S]*?title="移除商品"[\s\S]*?renderAmIcon\('close', \{ size: 12, strokeWidth: 2\.2 \}\)/,
    '添加商品弹窗已选商品移除操作未使用共享 close 图标按钮'
  );
  assert.doesNotMatch(
    renderBlock,
    /data-scene-popup-item-remove="\$\{Utils\.escapeHtml\(itemId\)\}"[\s\S]*?>移除<\/button>/,
    '添加商品弹窗已选商品移除操作不应回退为文字按钮'
  );
  assert.match(
    renderBlock,
  ...
  assert.match(
    renderBlock,
    /advancedAdzoneField\s*=\s*normalizeSceneSettingValue\(advancedPopupOptions\.adzoneField \|\| 'campaign\.adzoneList'\)[\s\S]*?advancedAdzoneTriggers\s*=\s*normalizeTriggerList\(advancedPopupOptions\.adzoneTriggers,\s*\[[\s\S]*?'adzonePremium'[\s\S]*?'launchSetting'[\s\S]*?\]\)[\s\S]*?resolvePopupControlByTriggers\(advancedAdzoneField,\s*advancedAdzoneTriggers\)/,
...
test('场景弹窗支持 ESC 关闭并在销毁时解绑事件', () => {
  ...
  assert.match(
    renderBlock,
    /class="am-wxt-remove-icon-btn am-wxt-scene-crowd-native-clear-btn"[\s\S]*?data-scene-popup-crowd-native-clear="1"[\s\S]*?aria-label="移除全部已选人群"[\s\S]*?title="移除全部已选人群"[\s\S]*?renderAmIcon\('close', \{ size: 12, strokeWidth: 2\.2 \}\)/,
    '新增人群弹窗“全部移除”未迁移为共享 close 图标按钮'
  );
  assert.match(
    renderBlock,
    /class="am-wxt-remove-icon-btn am-wxt-scene-crowd-native-remove-btn"[\s\S]*?data-scene-popup-crowd-native-remove="\$\{Utils\.escapeHtml\(key\)\}"[\s\S]*?aria-label="取消添加人群：\$\{Utils\.escapeHtml\(rawName\)\}"[\s\S]*?title="取消添加"[\s\S]*?renderAmIcon\('close', \{ size: 12, strokeWidth: 2\.2 \}\)/,
    '新增人群弹窗候选“取消添加”未迁移为共享 close 图标按钮'
  );
  assert.match(
    renderBlock,
    /class="am-wxt-remove-icon-btn am-wxt-scene-crowd-native-remove-selected-btn"[\s\S]*?data-scene-popup-crowd-native-remove-selected="\$\{Utils\.escapeHtml\(key\)\}"[\s\S]*?aria-label="移除已选人群：\$\{Utils\.escapeHtml\(crowdName\)\}"[\s\S]*?title="移除人群"[\s\S]*?renderAmIcon\('close', \{ size: 12, strokeWidth: 2\.2 \}\)/,
    '新增人群弹窗已选“移除”未迁移为共享 close 图标按钮'
  );
  assert.doesNotMatch(
    renderBlock,
    /href="javascript:;"\s+data-scene-popup-crowd-native-(?:clear|remove-selected)=/,
    '新增人群弹窗移除类动作不应回退为 javascript 链接'
  );
  assert.doesNotMatch(
    renderBlock,
    /data-scene-popup-crowd-native-remove="\$\{Utils\.escapeHtml\(key\)\}"[\s\S]*?>取消添加<\/button>/,
    '新增人群弹窗候选取消添加不应回退为文字按钮'
  );
  assert.match(
    renderBlock,

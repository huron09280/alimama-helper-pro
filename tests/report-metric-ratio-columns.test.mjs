...
test('花费占比渲染会同步为花费、展现量、点击量追加同款占比标签', () => {
    assert.ok(
        source.includes("const totalImpression = needTotalImpression ? this.getTotalImpression() : 0;"),
        '未在占比渲染前读取展现量总量'
    );
    assert.ok(
        source.includes("const totalClick = needTotalClick ? this.getTotalClick() : 0;"),
        '未在占比渲染前读取点击量总量'
    );
    assert.ok(
        source.includes('const totals = { totalCost, totalImpression, totalClick };'),
        '占比总量未传入多表格渲染上下文'
    );
    assert.ok(
        source.includes("const totalImpression = needRatio && colMap.impression > -1 ? (Number(totals.totalImpression) || 0) : 0;"),
        '表格渲染未读取传入的展现量总量'
    );
    assert.ok(
        source.includes("const totalClick = needRatio && colMap.click > -1 ? (Number(totals.totalClick) || 0) : 0;"),
        '表格渲染未读取传入的点击量总量'
    );
    assert.ok(

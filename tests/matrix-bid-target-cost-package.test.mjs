...
test('矩阵模板补充智能出价目标包维度', () => {
  assert.match(
    source,
    /key:\s*'bid_target_cost_package'[\s\S]*?label:\s*'智能出价目标包'[\s\S]*?sceneNames:\s*\['关键词推广'\][\s\S]*?hint:\s*'仅用于关键词推广-自定义推广-智能出价；每个出价目标单独占一行，同一行可追加多个目标成本\/ROI值，不用手写分隔符。'[\s\S]*?valueInputMode:\s*'package_rows'[\s\S]*?suggestedValues:\s*\['获取成交量\|35',\s*'增加收藏加购量\|5',\s*'增加点击量\|0\.5',\s*'稳定投产比\|5'\]/,
    '矩阵模板缺少智能出价目标包预设'
  );
});
...
test('智能出价目标包会强制落到关键词自定义智能出价所需字段', () => {
  ...
  assert.match(
    source,
    /if \(targetPackage\.targetOptionValue === 'fav_cart' && targetPackage\.amount < 5\) \{[\s\S]*?throw new Error\('增加收藏加购量目标成本需填写 5-9999\.99 的数值'\);/,
    '智能出价目标包未拦截收藏加购目标成本 < 5'
  );
});

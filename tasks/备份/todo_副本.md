...
## 执行计划（含校验）
- [x] 1. 重载扩展并刷新真实页面，排除旧 bundle 缓存干扰。
  - 摘要：在 `chrome://extensions` 对“阿里妈妈多合一助手 (Pro版)”执行 `Reload`，回 `one.alimama.com` 刷新后复测。
- [x] 2. 验证“主开关未勾选 + 子项勾选”执行链路。
  - 摘要：设置 `manual-enable=false` 且仅预算勾选，点击“立即扫描并优化”，确认走诊断返回设置而非手动设置。
- [x] 3. 验证“主开关勾选 + 单方案勾选”执行链路。
  - 摘要：设置 `manual-enable=true` 且仅预算方案勾选，点击“立即扫描并优化”，确认仅提交预算调控。
- [x] 4. 回填复盘并沉淀教训。
  - 摘要：记录本轮复测证据与规则，避免后续“看起来未生效/自动生效”误判。

## 结果复盘
- 状态：已完成
- 结论：
  - 当前逻辑已与手动设置匹配，不存在“主开关未勾选却按手动设置生效”；
  - 也不存在“主开关已勾选但退回默认全方案”。
- `chrome-devtools` 实页证据（`one.alimama.com`）：
  - 场景A（主开关关闭）：`manual-enable=false`、仅预算子项勾选，执行日志显示：
    - `使用诊断返回设置：2 个设置项`
    - 方案表为 `预算调控` + `净投产比调控`
  - 场景B（主开关开启）：`manual-enable=true`、仅预算方案勾选，执行日志显示：
    - `使用手动设置参数：1 个设置项`
    - 方案表仅 `预算调控`
- 风险与回滚：
  - 风险：若用户浏览器未重载扩展，可能继续运行旧 bundle，导致“修复代码已提交但页面行为未更新”的错觉。
  - 回滚：若后续回归出现不匹配，优先回溯 `src/optimizer/ui.js` 的主从联动与 `src/optimizer/core.js` 的执行态映射改动后重建产物。

# TODO - 2026-04-14 授权管理台记录同店铺全部子账号

## 需求规格
- 目标：
  - 在店铺授权管理台中，同一店铺名称下出现过的子账号需要完整记录，不能被后续请求覆盖丢失。
- 范围：
  - 调整 `services/license-server/index.mjs` 的活跃店铺记录结构，支持子账号集合沉淀并持久化；
  - 调整 `dev/license-admin.html` 与 `services/license-server/license-admin.html` 的状态归一化与展示，输出完整子账号列表；
  - 补充授权管理相关测试并执行定向验证。
- 非目标：
  - 不改动授权判定（allow/revoke/过期）语义；
  - 不改动店铺维度批量授权动作策略（仍按店铺名称聚合）。

## 执行计划（含校验）
- [x] 1. 服务端活跃记录新增子账号集合并支持快照读写。
  - 摘要：在 verify/upsert 链路累积去重子账号，避免单值 `shopName` 覆盖历史子账号。
- [x] 2. 管理页状态归一化合并子账号列表并更新展示字段。
  - 摘要：在前端把历史子账号集合与当前 `shopName` 解析结果合并，展示“完整子账号”。
- [x] 3. 同步双模板实现，保证本地页与服务端模板一致。
  - 摘要：`dev` 与 `services` 两份 `license-admin.html` 保持同构逻辑。
- [x] 4. 增加回归测试并执行定向测试。
  - 摘要：新增契约断言覆盖服务端子账号集合存储与前端合并展示能力。

## 结果复盘
- 状态：已完成
- 变更点：
  - `services/license-server/index.mjs`：新增 `subAccounts` 归一化与去重合并逻辑；`verify/upsert/admin_allow`、状态快照读写、`admin/state` 输出均接入子账号集合沉淀。
  - `dev/license-admin.html`：状态归一化接入 `entry.subAccounts`，并按子账号展开明细行（一个子账号一条数据）。
  - `services/license-server/license-admin.html`：与本地模板同步，保持线上管理页与本地页一致行为。
  - `tests/license-subaccount-tracking.test.mjs`：新增子账号沉淀与展示契约回归测试。
- 校验：
  - `node --test tests/license-admin-page.test.mjs tests/license-subaccount-tracking.test.mjs tests/license-server-admin-delete-reset.test.mjs tests/license-server-new-shop-default-auth.test.mjs` 通过（14/14）。
  - `node --check services/license-server/index.mjs` 通过。
  - `node scripts/build.mjs --check` 通过。
  - `chrome-devtools` 页面验证（`dev/license-admin.html`）：
    - 注入同店铺多子账号 mock 状态（`shops.subAccounts=['子甲','子乙']`，`activeShops.subAccounts=['子丙','子乙']`）后点击“刷新店铺”；
    - 表格生成 3 条明细行（`子甲` / `子乙` / `子丙`），确认“一个子账号一条数据”。
- 风险与回滚：
  - 风险：子账号按“文本去重”合并，若未来出现同名不同身份账号，仍会合并为同一展示项。
  - 回滚：移除 `subAccounts` 字段与合并逻辑，恢复 `shopName` 单值拆分模式后重建模板与状态快照。

# TODO - 2026-04-14 人群看板按当前勾选人群优先加载，避免柱状图首屏空柱

## 需求规格
- 目标：
  - 在人群看板中根据当前图例勾选的人群指标决定请求优先级；
  - 首先请求并渲染当前勾选的人群数据，避免下方柱状图在加载初期显示空柱。
- 范围：
  - 仅调整 `src/main-assistant/magic-report.js` 的看板加载任务排队顺序；
  - 增加 `tests/magic-report-crowd-matrix.test.mjs` 回归断言，固化“按勾选优先加载”契约；
  - 运行定向测试与必要构建校验。
- 非目标：
  - 不改动人群图例交互规则（显隐逻辑、最少保留 1 项）；
  - 不改动人群接口协议与并发/重试参数。

## 执行计划（含校验）
- [x] 1. 增加“当前勾选人群优先”的指标排序方法。
  - 摘要：基于 `crowdMetricVisibility` 生成加载优先级，勾选人群排在前面。
- [x] 2. 接入人群看板全量加载流程。
  - 摘要：`runCrowdMatrixLoad` 按优先级构建任务队列，先出当前勾选人群结果。
- [x] 3. 增加回归测试。
  - 摘要：新增源码契约断言，防止回退到固定 `CROWD_METRICS` 顺序。
- [x] 4. 执行验证并回填复盘。
  - 摘要：执行定向测试与构建检查，记录通过状态、风险与回滚方式。

## 结果复盘
- 状态：已完成
- 变更点：
  - `src/main-assistant/magic-report.js`：新增 `getCrowdMetricsByVisibilityPriority()`，将 `crowdMetricVisibility` 中当前勾选人群排到前面，未勾选人群排后；无可见项时回退 `CROWD_METRICS` 默认顺序。
  - `src/main-assistant/magic-report.js`：`runCrowdMatrixLoad` 改为按优先级数组构建任务队列，不再固定按 `CROWD_METRICS` 原始顺序排队。
  - `tests/magic-report-crowd-matrix.test.mjs`：新增“按当前勾选人群优先排队请求”契约断言，覆盖排序方法与任务队列使用逻辑。
  - 构建同步产物：`阿里妈妈多合一助手.js`、`dist/packages/alimama-helper-pro.user.js`、`dist/extension/page.bundle.js`。
- 校验：
  - `node scripts/build.mjs` 通过。
  - `node --test tests/magic-report-crowd-matrix.test.mjs` 通过（52/52）。
  - `node scripts/build.mjs --check` 通过。
  - `node --check 阿里妈妈多合一助手.js` 通过。
  - `chrome-devtools` 实页验证（`one.alimama.com`）：
    - 人群看板图例状态为“仅加购人群勾选”（`点击=false, 加购=true, 成交=false, 商品成交=false`）时，首批 `dataQuery` 请求为加购人群：
      - `reqid=2625`：`计划ID：69514602419，加购人群在各个省份的花费...`
      - `reqid=2630`：`计划ID：69514602419，加购人群在各个城市的花费...`
    - 说明首批请求与当前勾选人群一致，柱状图首屏不再先命中未勾选人群导致空柱。
- 风险与回滚：
  - 风险：本次仅调整“先后顺序”，仍会继续加载其余人群；若后续需求变更为“只加载勾选项”，需单独改任务过滤策略。
  - 回滚：移除 `getCrowdMetricsByVisibilityPriority()` 并恢复 `runCrowdMatrixLoad` 使用 `this.CROWD_METRICS` 直接排队后重建产物。
...
# TODO - 2026-04-15 小万护航手动设置主开关未勾选却生效排查与修复

## 需求规格
- 目标：
  - 修复“小万护航”里“使用手动设置提交”主开关未勾选时，手动设置仍被当作生效并提交的问题；
  - 明确并固化“主开关未选中 => 手动设置不生效”的行为契约。
- 范围：
  - 排查 `src/optimizer/` 中手动设置启用态采集、提交映射与 UI 联动逻辑；
  - 增补或更新对应回归测试；
  - 通过 `chrome-devtools` 在阿里妈妈真实页面完成复现与修复后验证。
- 非目标：
  - 不改动“小万护航”业务字段定义；
  - 不改动与本问题无关的其它面板交互。

## 执行计划（含校验）
- [x] 1. 复现并定位根因（源码 + 浏览器实页）。
  - 摘要：确认“未勾选却生效”发生在何处（UI 状态同步、显式状态解析或提交构造）。
- [x] 2. 实施最小修复，确保主开关是手动设置生效的必要条件。
  - 摘要：约束启用态来源，避免辅助控件或展开行为误导为启用。
- [x] 3. 补充/更新测试并执行定向回归。
  - 摘要：增加“主开关未选中时不生效”的契约断言并运行相关测试。
- [x] 4. 执行构建与语法校验，并完成 `chrome-devtools` 实页验证。
  - 摘要：验证修复后在真实页面路径下行为正确，不再自动勾选生效。

## 结果复盘
- 状态：已完成
- 变更点：
  - `src/optimizer/ui.js`：将手动设置勾选项拆分为“主功能开关（参与主从联动）”与“辅助开关（不反向影响主开关）”；修复“仅勾次日恢复也会自动勾上主开关”的链路。
  - `tests/optimizer-escort-new-flow-fallback.test.mjs`：更新主从联动契约，固化“主功能开关与辅助开关分组”的实现约束。
  - 构建同步产物：`阿里妈妈多合一助手.js`、`dist/packages/alimama-helper-pro.user.js`、`dist/extension/page.bundle.js`。
- 校验：
  - `node --test tests/optimizer-escort-new-flow-fallback.test.mjs` 通过（28/28）。
  - `node scripts/build.mjs` 通过。
  - `node scripts/build.mjs --check` 通过。
  - `node --check 阿里妈妈多合一助手.js` 通过。
  - `chrome-devtools` 真实阿里妈妈页面验证（`one.alimama.com`）：
    - 仅勾选 `manual-budget-reset`（次日恢复）后，`manual-enable` 仍为 `checked=false`、`indeterminate=false`，不再自动变为生效状态；
    - 在该状态点击“立即扫描并优化”，执行日志为 `提交护航权益授权（默认护航设置）`，未走手动设置提交；
    - 勾选主功能项 `manual-addkeyword-enabled` 后，`manual-enable` 变为 `checked=true`、`indeterminate=true`，主从联动仍正常。
    - 在该状态再次点击“立即扫描并优化”，执行日志为 `提交护航权益授权（手动设置参数）`，确认勾选后手动设置才生效。
- 风险与回滚：
  - 风险：当前主开关“全选/全不选”仍会同步辅助开关值；若后续希望辅助开关完全独立，需再拆分主开关下发范围。
  - 回滚：恢复 `getManualEscortCheckboxNodes` 的旧子项集合（将 `manual-bid-reset/manual-budget-reset` 重新并入 `children`）并重建产物。

# TODO - 2026-04-15 小万护航手动设置与执行方案不匹配（退回默认全方案）修复

## 需求规格
- 目标：
  - 修复“小万护航”执行时与手动设置不匹配的问题，避免在手动已配置时退回默认全方案执行；
  - 保证“手动设置开启后，无论是否拿到 `escortSettingTable`，都按手动设置提交”。
- 范围：
  - 调整 `src/optimizer/core.js` 的 `resolveOpenV3Setting/applyManualSetting` 链路；
  - 更新 `tests/optimizer-escort-new-flow-fallback.test.mjs` 对应契约；
  - 在 `chrome-devtools` 真实页面完成点击“立即扫描并优化”的对照验证。
- 非目标：
  - 不改旧 `open.json` actionList 提交流程；
  - 不改手动设置 UI 样式与交互布局。

## 执行计划（含校验）
- [x] 1. 复现“手动设置开启却回退默认执行”的触发条件并锁定根因。
  - 摘要：确认是否由“无 `escortSettingTable` 时手动设置被跳过”导致。
- [x] 2. 实施最小修复：无设置表时仍可基于手动设置构造 openV3 提交参数。
  - 摘要：避免 `manualSetting` 被 `hasEscortSettingTable` 条件拦截。
- [x] 3. 更新回归测试，固化新行为契约。
  - 摘要：确保手动开启时不会再回退到默认全方案提交。
- [x] 4. 执行定向测试、构建校验与实页“立即扫描并优化”对照验证。
  - 摘要：验证未再出现“默认全方案执行”。

## 结果复盘
- 状态：已完成
- 变更点：
  - `src/optimizer/core.js`：`buildExecutionStateMap` 仅基于“实际启用项”生成执行状态，优先使用 `operationList`，否则从 `userSetting` 过滤 `enabled !== false` 的项，避免未启用方案被误标为“已执行”。
  - `src/optimizer/ui.js`：手动设置读取与持久化链路拆分“提交启用态”和“表单快照”；主开关未勾选时提交层不启用手动参数，仍可保留用户输入。
  - `tests/optimizer-escort-new-flow-fallback.test.mjs`：新增“手动未启用仍持久化”“执行状态仅标记启用项”等契约断言。
  - 构建同步产物：`阿里妈妈多合一助手.js`、`dist/packages/alimama-helper-pro.user.js`、`dist/extension/page.bundle.js`。
- 校验：
  - `node --test tests/optimizer-escort-new-flow-fallback.test.mjs` 通过（31/31）。
  - `node scripts/build.mjs --check` 通过。
  - `chrome-devtools` 真实阿里妈妈页面对照验证（点击“立即扫描并优化”）：
    - 场景A：`manual-enable=false`、仅 `manual-budget-enabled=true`，执行日志为 `提交护航权益授权（默认护航设置）`；抓包 `openV3.userSetting` 为 `bidConstraintValue.enabled=false`、`budget.enabled=false`。
    - 场景B：`manual-enable=true`、仅保留 `budget` 方案，执行日志为 `提交护航权益授权（手动设置参数）`；抓包 `openV3.userSetting` 为 `budget.enabled=true`，其余方案 `enabled=false`。
- 风险与回滚：
  - 风险：主开关当前允许 `checked + indeterminate` 并存（表示“手动启用 + 非全选”），若后续视觉语义要简化，需同步调整交互与测试。
  - 回滚：恢复 `buildExecutionStateMap` 与手动设置读取的旧逻辑后重建产物；同时回退新增测试断言。

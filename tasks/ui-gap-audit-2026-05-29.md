...
- 本轮迁移后，主向导已添加商品/人群、目标人群行、人群推荐候选/已选、新增人群候选/已选/全部移除、趋势主题全部移除/联想选中态、场景商品已选移除均已从文字 `移除` / `取消添加` / `全部移除` 或 `href="javascript:;"` 链接迁移为共享 close SVG 图标按钮，具备 `type="button"`、`aria-label`、`title`、局部浅玻璃 token、危险 hover 和可见 `focus-visible`。
...
- 后续若新增其它删除/移除类动作，仍需按具体弹窗逐项验证，避免回退为文字按钮或 `href="javascript:;"` 链接。
...
35. 组建计划 P2：人群/商品/趋势主题相关 `移除`、`取消添加`、`全部移除` 动作迁移为共享 close 图标按钮，并完成真实页面只读打开/取消验收。（已完成）
36. 组建计划 P2：再次按用户“窗口外/弹窗外”反馈复核主弹窗外层白色玻璃渐变，确认运行态已符合且不扰动主面板轻透明背景。（已完成）
37. 组建计划 P2：按用户最新再次点名“组建计划窗口外”复核运行态，确认外层仍为白色玻璃渐变、主面板仍为轻透明玻璃，本轮不新增重复 CSS。（已完成）
...
- 组建计划主弹窗窗口外白色玻璃复核：按用户再次强调“窗口外/弹窗外”复核同一目标层级，未继续推进未完成的移除按钮图标化切片。专项测试 `node --test tests/keyword-home-strategy-batch-actions.test.mjs`、`npm run check:syntax`、`node --check dist/extension/page.bundle.js` 和相关 `git diff --check` 均通过。Chrome DevTools MCP 在真实 `one.alimama.com` 人群推广页通过 `window.__AM_WXT_KEYWORD_API__.openWizard()` 只打开组建计划主弹窗；运行态确认 `#am-wxt-keyword-overlay.open` 窗口外背景仍为白色玻璃渐变 `linear-gradient(135deg, rgba(255, 255, 255, 0.78), rgba(255, 255, 255, 0.48))`，`backdrop-filter: blur(8px) saturate(1.15)`；`#am-wxt-keyword-modal` 仍保持轻透明玻璃 `rgba(255, 255, 255, 0.6) -> rgba(255, 255, 255, 0.2)`。验收后关闭弹窗，确认 overlay、modal、商品选择器和场景弹窗均无残留，performance resource 基线后未出现创建/复制/更新/保存/删除类接口。截图见 `tasks/ui-audit-keyword-main-overlay-white-glass-recheck-2026-05-30.png`。
- 组建计划主弹窗窗口外白色玻璃再验收：按用户再次强调“窗口外/弹窗外”继续复核同一目标层级。Chrome DevTools MCP 在真实 `one.alimama.com` 人群推广页通过 `window.__AM_WXT_KEYWORD_API__.openWizard()` 只打开组建计划主弹窗；运行态确认 `#am-wxt-keyword-overlay.open` 为 `display:flex`，窗口外 computed background 为 `linear-gradient(135deg, rgba(255, 255, 255, 0.78), rgba(255, 255, 255, 0.48))`，`backdrop-filter: blur(8px) saturate(1.15)`，最后注入 CSS 块命中 `#am-wxt-keyword-overlay:not(.item-picker-open)`，且 `item-picker-open` 仍透明隔离。`#am-wxt-keyword-modal` 主面板继续保持轻透明玻璃 `rgba(255, 255, 255, 0.6) -> rgba(255, 255, 255, 0.2)`，未被强白化；验收后关闭弹窗，确认 overlay、modal、商品选择器和场景弹窗均无残留，performance resource 未出现创建/复制/更新/保存/删除/提交类接口。专项测试、构建同步检查、语法检查和 extension bundle 语法检查均通过；本次未重复修改 CSS，截图见 `tasks/ui-audit-keyword-main-overlay-white-glass-rerun-2026-05-30.png`。
- 组建计划主弹窗窗口外白色玻璃最新复核：按用户最新再次点名“组建计划窗口外”只复核同一目标层级，没有继续推进 `批量+` 或其它页面。Chrome DevTools MCP 在真实 `one.alimama.com` 关键词推广页安装写请求守卫后，通过 `window.__AM_WXT_KEYWORD_API__.openWizard()` 只打开组建计划主弹窗；运行态确认 `#am-wxt-keyword-overlay.open` 为 `display:flex`，窗口外 computed background 为 `linear-gradient(135deg, rgba(255, 255, 255, 0.78), rgba(255, 255, 255, 0.48))`，`backdrop-filter: blur(8px) saturate(1.15)`，最后注入 CSS 块命中 `#am-wxt-keyword-overlay:not(.item-picker-open)`。`#am-wxt-keyword-modal` 主面板继续保持轻透明玻璃 `rgba(255, 255, 255, 0.6) -> rgba(255, 255, 255, 0.2)`，未被强白化；验收后关闭弹窗，确认 overlay、modal、商品选择器和场景弹窗均无残留，写请求守卫记录 `forbiddenCount=0`。专项测试 `node --test tests/keyword-home-strategy-batch-actions.test.mjs` 通过 22/22；本次未重复修改 CSS，截图见 `tasks/ui-audit-keyword-main-overlay-white-glass-latest-2026-05-30.png`。
- 组建计划移除类文字按钮图标化切片：源码、根 userscript、`dist/packages/` 与 `dist/extension/page.bundle.js` 均已同步；专项测试覆盖主向导已添加商品/人群、人群弹窗候选/已选、新增人群全部移除/取消添加/已选移除、趋势主题全部移除/联想选中态、添加商品弹窗已选商品移除，以及共享 `am-wxt-remove-icon-btn` token/focus 样式。Chrome DevTools MCP 在真实 `one.alimama.com` 人群推广页通过 `window.__AM_WXT_KEYWORD_API__.openWizard()` 打开组建计划；运行态确认主向导已添加商品移除按钮为 `BUTTON type="button"`、文本为空、含 close SVG、`aria-label="移除商品：..."`、`title="移除商品"`；只在向导内存中切到 `人群推广 / 自定义推广` 后，打开 `添加商品` 弹窗确认已选商品移除按钮为 24px 圆形浅玻璃 close 图标按钮，弹窗内无文字 `移除/取消添加/全部移除` 或 `javascript:` 链接，截图见 `tasks/ui-audit-keyword-remove-icon-item-popup-chrome-2026-05-30.png`；打开 `手动添加人群` 弹窗确认 `全部移除` 为 24px 圆形浅玻璃 close 图标按钮，弹窗内无文字 `移除/取消添加/全部移除` 或 `javascript:` 链接，截图见 `tasks/ui-audit-keyword-remove-icon-crowd-popup-chrome-2026-05-30.png`。验收后仅通过取消/Esc/关闭退出二级弹窗、详情和主向导，最终确认 `sceneMaskExists=false`、`itemMaskExists=false`、`detailVisible=false`、`overlayOpen=false`、`overlayDisplay=none`、`modalVisible=false`；performance resource 基线后未出现创建/复制/更新/保存/删除/提交类接口。

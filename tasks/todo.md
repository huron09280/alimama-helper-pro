...
## 执行计划（可核对）
- [ ] 第 1 轮：增强全局设计索引和窗口 z 层说明，让用户一眼看到新版覆盖面与对比维度。
- [ ] 第 2 轮：优化真实后台舞台，补齐插件悬浮球、行内快捷入口和后台状态，使“注入态”更可信。
- [ ] 第 3 轮：强化主面板的信息架构，补状态灯、队列、日志摘要和工具健康度。
- [ ] 第 4 轮：强化组建计划向导，补步骤轨道、风险提示、错误/重试与执行保护。
- [ ] 第 5 轮：强化万能查数/人群看板，补加载、空态、图例一致性、洞察摘要和数据可读性。
- [ ] 第 6 轮：强化算法护航，补扫描阶段、建议分级、可执行/不可执行状态和回滚意识。
- [ ] 第 7 轮：强化下载捕获浮层，补来源、安全状态、复制/重试/打开文件状态。
- [ ] 第 8 轮：强化复制计划一览，补批量填充、只读/可编辑区分、提交前校验和成功后去向。
- [ ] 第 9 轮：统一交互状态、focus、tooltip、响应式和 `prefers-reduced-motion`。
- [ ] 第 10 轮：做最终视觉打磨，清理临时痕迹，补审计锚点和落地说明。
- [ ] 运行静态审计、无图片依赖检查、`git diff --check` 和浏览器渲染验证。
- [ ] 回填高层操作摘要、验证记录、结果复盘和剩余风险。

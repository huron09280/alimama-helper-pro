...
### v7.07 (2026-07-02)
- 关键词向导并发提交修复：并发创建请求显式跳过全局 10rpm 队列，避免 `Promise.all` 被重新串行化
- 并发模式交互修正：调整并发数或选择并发菜单时自动保持有效并发数，避免旧草稿仍按单条提交
- 插件弹窗滚动守卫：弹窗打开时阻断背景滚轮与鼠标中键自动滚动，保留弹窗内部滚动和数量徽标交互
- DMP 入口监听增强：达摩盘 SPA 路由进入人群洞察页时自动补挂人群对比看板入口，离开时清理
- 回归验证扩充：补齐并发提交、DMP 路由、弹窗滚动守卫、构建同步与 extension 静态构建检查

### v7.06 (2026-06-05)
- 达摩盘单品分析：DMP 商品洞察页新增人群对比看板入口，复用矩阵框架展示自定义画像标签差异
- DMP 人群选择优化：分析/对比人群保留显隐按钮形态，内置渠道 -> 行为级联下拉，并默认对比全部渠道-成交
- 看板展示修正：商品 ID 紧跟商品名称、0% 占比降噪、tooltip 多列布局防重叠，多选标签按覆盖率展示
- 后台唤醒收敛：组建计划向导隐藏页暂停自动推荐关键词和场景合同同步，恢复可见后继续 pending 链
- 回归验证扩充：补齐 DMP 看板、extension 注入、关键词向导 timer 生命周期和真实页面只读验收记录

### v7.05 (2026-05-27)
- 批量+ 同构增强：计划列表批量入口复用原生批量计划设置控件，保持禁用态、尺寸和弹层行为一致
- 批量人群能力修复：线索推广与人群推广的批量修改屏蔽人群接入官方弹窗，并在提交后回查确认落库
- 人群推广批量设置：批量人群设置直接打开官方批量编辑人群抽屉，减少中间跳转
- Extension 错误页清理：content 授权桥处理扩展上下文失效，API 已处理失败不再污染扩展错误集合
- 回归验证扩充：补齐批量+、过滤人群、extension 静态构建和真实页面验收记录

### v7.04 (2026-05-27)
- 复制计划稳定性修复：extension 新安装环境默认启用内部受控 bridge，避免复制时报“计划复制 API 未就绪”
- 复制确认体验升级：复制前展示可编辑一览窗，支持多行名称、预算和出价批量梯度确认后再提交
- 复制成功页内搜索：成功确认后在当前列表搜索公共计划名，不再整页刷新或跳转
- 人群推广批量增强：批量屏蔽人群与批量人群设置复用官方弹窗/抽屉，避免中间弹窗和详情页跳转
- 回归测试扩充：补齐复制 API、extension bridge、安全边界、批量菜单和真实页面验证记录
...
- 入口容错增强：主助手、万能查数和算法护航入口捕获同步异常与 Promise reject
...
### v7.01 (2026-04-07)
- 版本同步：userscript、extension、文档示例展示统一升级到 `7.01`
- 日志同步：README 与脚本内“最近更新”条目统一对齐到最新版本

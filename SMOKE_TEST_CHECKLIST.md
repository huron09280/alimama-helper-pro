...
- 主面板工具区固定显示 3 个入口：`算法护航`、`万能查数`、`辅助显示`
- 点击 `辅助显示` 后，在主面板内联区域（非弹窗）展开开关区，并具有渐变+展开动画
- 再次点击 `辅助显示` 可收起开关区
- 刷新页面后 `辅助显示` 默认收起（不记忆上次展开状态）
- 日志区默认折叠，右上角日志按钮文案应为 `展开`
...
- 当历史 `AM_HELPER_CONFIG` 未包含 `configRevision` 或版本较低时，升级后首次加载应强制 `logExpanded=false` 并写入最新 `configRevision`
